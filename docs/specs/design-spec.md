...
| `GET` | `/api/boards/:bid` | one board, and the ONLY route that answers with a task's `log`. Every other task-shaped response — the board list, the task `POST`/`PATCH`, the SSE frame — carries the log-free `TaskFrame`. |
