...
| `GET` | `/api/boards/:bid` | one board, and the ONLY route that answers with a task's `log`. Every other task-shaped response — the board list, the task `POST`/`PATCH`, the SSE frame — carries the log-free `TaskFrame`. The web reads it once, when a card's Log tab opens (`web/src/components/TaskLogTab.tsx`), and Zod-parses the answer. The board's per-card badge reads only the frame's counters; "N new" is per viewer, remembered in the browser (`web/src/lib/log-seen.ts`) from what the Log tab last displayed — the server holds no seen notion. |
| `POST` | `/api/boards/:bid/tasks/:tid/log` | append ONE entry to the card's log. Its own route, not a field on the task `PATCH`: a `PATCH` carries last-write-wins semantics for `description`, which is exactly what an append-only field must not inherit. **`atMs` and the writer are server-assigned** (a request body timestamp is ignored). The writer is resolved from `{env,paneId}` across **every** board, so a session bound to ANOTHER card may append here (the §4 invariant permits adding to a card you are not bound to); it refuses `403` only when the pane is bound to no card anywhere — there is then no session to name. Entry timestamps are epoch MILLIS — the task's own `createdAt`/`updatedAt` are seconds, so the field name carries the unit. The log is **not** in the SSE frame — that carries `logCount`/`noteCount`/`lastLogAtMs` only (`noteCount` because every card carries lifecycle entries from birth, so `logCount` cannot say whether a session has written anything). |
| — | **system log entries** | Besides `note` (the one kind a session writes through the route above), corral stamps its own lifecycle entries — `created` (task create / from-session), `session_spawned` (spawn), `session_bound` (attach), `session_detached` (detach), `session_closed` (close), `status_changed` (task `PATCH` / move, only on an actual column change) — on the routes that already mutate the board. Their `source` is `corral`, the session is named in the entry text, and they carry a SEPARATE quota from notes so a burst of lifecycle lines cannot evict the reasoning. |
