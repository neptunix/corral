...
# Metrics only — no name. corral reads a session's name from Claude's registry (server/session-registry.ts),
# which is live rather than written on statusline render, so a name captured here would be stale for any
# idle session. This script no longer reads the registry at all.
