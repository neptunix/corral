...
function task(over: Partial<EnrichedTask>): EnrichedTask {
  return { id: "t1", title: "Card", description: "", status: "c1", priority: null, sessions: [], createdAt: 0, updatedAt: 0, logCount: 0, lastLogAtMs: null, ...over };
}
...
describe("TaskCard — log badge", () => {
  it("a card never opened here shows every entry as new", () => {
    render(card(task({ logCount: 9, lastLogAtMs: 5_000 })));
  ...
  it("entries written since the last look count as new — and the mark is per card, not per board", () => {
    ...
    render(card(task({ logCount: 9, lastLogAtMs: 6_000 })));
    expect(screen.getByText("3 new")).toBeTruthy();
  });

  it("a session on the card and nothing in the log is the card-signal case: \"nothing written\"", () => {
    render(card(task({ sessions: [live] })));
  ...
  it("clicking the badge opens the log, not the card's session or the edit form", () => {
    ...
    const onEdit = vi.fn();
    render(card(task({ logCount: 2, lastLogAtMs: 1 }), onOpenLog, onEdit));
